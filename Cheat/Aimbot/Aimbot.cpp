...
void CAimbot::AimbotSet()
{
	...
	if (Settings::Aimbot::aim_Psilent == 1)
	{
		QAngle oldView = m_pCmd->viewangles;
		float oldSidemove = m_pCmd->sidemove;
		float oldForwardmove = m_pCmd->forwardmove;

		// Do all aimbot / nospread / norecoil stuff here1x
		...
		float flServerTime = m_pLocal->m_pEntity->GetTickBase() * Interfaces::GlobalVars()->interval_per_tick;
		float flNextPrimaryAttack = m_pLocal->m_pWeaponEntity->GetNextPrimaryAttack();

		bool bBulletTime = true;
		if (flNextPrimaryAttack > flServerTime)
			bBulletTime = false;

		if (m_pCmd->buttons & IN_ATTACK && bBulletTime) {
			*bSendPacket = false;
		}
		else {
			*bSendPacket = true;
			m_pCmd->viewangles = oldView;
			m_pCmd->sidemove = oldSidemove;
			m_pCmd->forwardmove = oldForwardmove;
		}
